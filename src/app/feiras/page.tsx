...
export default function Feiras() {
  ...
  const onSubmit = (data: any) => {
    setFeirasList([
      ...
      {
        ...data,
        id: feirasList.length
          ? Math.max(...feirasList.map((feira) => feira.id)) + 1
          : 1,
      },
    ]);
                    ...
                    <input
                      {...register("date", {
                        onChange(e: React.ChangeEvent<HTMLInputElement>) {
                          ...
                          setValue("feiraDays", { ...feiras });
                          setValue("exercise", String(year));
                        },
                      })}
                      type="month"
                      pattern="yyyy-mm"
                  ...
                  <tbody>
                    {feirasList.map((feira) => (
                      <tr key={feira.id}>
                        ...
                        <td>
                          {new Date(
                            Number(feira.exercise),
                            Number(feira.date.slice(5)),
                            0
                          )
                            .toISOString()
                            .slice(0, 10)
